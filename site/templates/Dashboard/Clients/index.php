	...
	<div class="row">
		<div id="clientsIndex" class="col">
			<?php foreach ($clients as $key => $client) : ?>
				<div class="box row" data-patient-id="<?=$client->patient_id?>">
					<div class="client col-md-6">
						...
						<div class="info">
							<a href="<?=$this->Url->build(['controller'=>'Clients','action'=>'view',$client->patient_id, 'prefix'=>'Dashboard']);?>">
								...
								<?php if ($client->patient->profile->gender): ?> <p><?=$genders[$client->patient->profile->gender]?> <i class="dot"></i> <?php endif; ?> <?php if ($client->patient->profile->pronoum): ?> <?=$pronouns[$client->patient->profile->pronoun]?> <i class="dot"></i> <?php endif; ?> <?=$client->patient->profile->age?></p>
							</a>
							<p class="mb-0">
								<?php
					...
					<div class="next_therapy col-md-4">
						<?php if ($client->status==2) : ?>
							<p class="mb-0 bold">Próxima terapia</p>
							<?php if ($client->patient->appointments_received && $client->patient->appointments_received[0]) { ?>
							...
							<?php } ?>
						<?php else : ?>
							<p class="mb-0 bold">Terapia encerrada</p>
						<?php endif;?>
					</div>
					<div class="contacts col-md-2">
						<?php if ($client->status==2) : ?>
							...
							<div class="d-none">
								<div class="custom-popover__content" id="popover-more_vert_<?=$key?>">
									<ul class="list--clean-style content-popover-more_vert_<?=$key?>">
										<li><a data-open-id-modal="#reportPatientModal" data-profile="psychologist" data-name="<?=$client->patient->name?>" data-id="<?=$client->patient->id?>" class="body1 popover__anchor" href="#">Denunciar cliente</a></li>
										<li><a data-open-id-modal="#closeTherapyModal" data-profile="psychologist" data-name="<?=$client->patient->name?>" data-id="<?=$client->patient->id?>" class="body1 popover__anchor" href="#">Encerrar terapia</a></li>
									</ul>
								</div>
							</div>
						<?php endif;?>
